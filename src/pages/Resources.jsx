    ...
    <div className={`min-h-screen ${isDarkMode ? 'dark bg-white' : 'bg-gradient-to-b from-blue-400 to-blue-600'}`}>
      ...
      <div className="flex flex-col items-center justify-center h-full">
        ...
        <div className="w-full sm:w-11/12 lg:w-4/5 xl:w-3/5">
          <h1 className={`text-5xl mb-4 font-bold text-center my-6 py-6 ${isDarkMode ? 'text-white' : 'text-black'}`}>
            Resources
          </h1>
      ...
      <footer className={`text-sm text-center ${isDarkMode ? 'text-white' : 'text-black'}`}>
        &copy; 2023 Studiously Inc. All Rights Reserved |{' '}
        <a href="/AboutUs" className={isDarkMode ? 'text-white hover:underline' : 'text-black hover:underline'}>About Us</a> |{' '}
        <a href="/terms-and-conditions" className={isDarkMode ? 'text-white hover:underline' : 'text-black hover:underline'}>Terms and Conditions</a>
      </footer>
    </div>
  );
};
...
const ResourceCard = ({ title, description, isDarkMode }) => {
  return (
    <div className={`w-full sm:w-1/2 lg:w-1/2 xl:w-1/3 p-4`}>
      <div className={`border-2 border-white rounded-lg p-4 shadow-md ${isDarkMode ? 'text-white' : 'text-black'}`}>
